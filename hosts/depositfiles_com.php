...
class dl_depositfiles_com extends Download { 
	...
	public function CheckAcc($cookie){
		//echo $data = $this->lib->curl("http://dfiles.eu/gold/payment_history.php", $cookie, ""); die();
	//	$data = $this->lib->curl("http://dfiles.eu/gold/payment_history.php", $cookie, "");
		$data = $this->lib->curl("http://depositfiles.com/gold/payment_history.php", $cookie, "");
		if(stristr($data, 'You have Gold access until:'))  {
			//$checksubscribe = $this->lib->curl("http://dfiles.eu/gold/payment_subscribe_manage.php", $cookie, "");
		//	$checksubscribe = $this->lib->curl("http://depositfiles.com/gold/payment_subscribe_manage.php", $cookie, "");
			...
			return array(true, "(fix by hitpro)<br/>Until ".$this->lib->cut_str($data, '<div class="access">You have Gold access until: <b>','</b></div>'));
		}
	...
	public function Login($user, $pass){	
		$data = $this->lib->curl("http://depositfiles.com/api/user/login?login=".urlencode($user)."&password=".urlencode($pass), '', '');
	...
	public function Leech($url) {
		...
		$data = $this->lib->curl("http://depositfiles.com/api/download/file?&file_id=".$DFid."&file_password=".urlencode($test), $this->lib->cookie, "", 0);
		...
		elseif($page['status'] == "OK" && isset($page['data']['download_url'])) return trim(str_replace("depositfiles.com", "dfiles.eu", $page['data']['download_url']));
